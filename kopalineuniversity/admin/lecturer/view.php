...
?> 
 ...
 <h2 class="page-header"> <small>Lecturer Name: </small><?php echo $singleuser->ACCOUNT_NAME; ?></h2>
 <h3>Lecturer Subjects</h3>
        ...
        <table id="dash-table" class="table table-striped table-bordered table-hover table-responsive" style="font-size:12px" cellspacing="0">
          ...
          <tbody>
            <?php  //`SUBJ_ID`, `SUBJ_CODE`, `SUBJ_DESCRIPTION`, `UNIT`, `PRE_REQUISITE`, `COURSE_ID`, `AY`, `SEMESTER`
              ...
              $mydb->setQuery("SELECT * FROM  tblsection sec, `tbllecturersubject` i,`subject` s, `course` c WHERE sec.SECTIONID=i.SECTIONID AND i.`SUBJ_ID`=s.`SUBJ_ID` AND s.COURSE_ID=c.COURSE_ID  AND ACCOUNT_ID='{$USERID}'");
