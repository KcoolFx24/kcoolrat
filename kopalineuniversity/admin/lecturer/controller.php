...
switch ($action) {
	...
	function doInsert(){
		...
		if (isset($_POST['selector'])==''){
			...
			for($i=0;$i<$key;$i++){ 
				...
				// $sql = "SELECT * FROM `tbllecturersubject` WHERE `SUBJ_ID`='{$id[$i]}'";
				// $mydb->setQuery($sql);
				...
				// if ($maxrow < 1) {
					# code...
					$sql ="INSERT INTO  `tbllecturersubject` (`ACCOUNT_ID`, `SUBJ_ID`,`SECTIONID`, `AY`) VALUES ('{$userid}','{$id[$i]}','{$section}','{$ay}')";
					$mydb->setQuery($sql);
					$mydb->executeQuery();
	...
	if(isset($_POST['save'])){
		...
		if ($_POST['INST_NAME'] == "" OR $_POST['INST_MAJOR'] == "" OR $_POST['INST_CONTACT'] == "" ) {
			...
			message("Lecturer has been updated!", "success");
			redirect("index.php");
	...
	function doDelete(){
		global $mydb;
	 	...
	 	$sql = "SELECT * FROM `tbllecturersubject` WHERE `INST_ID`=$id";
		$mydb->setQuery($sql);
		$res = $mydb->loadSingleResult();

			 $sql ="DELETE FROM `tbllecturersubject` WHERE `INST_ID`=$id";
			 $mydb->setQuery($sql);
			 $mydb->executeQuery();
